...
menuBtn.addEventListener('click', () => { sidebar.classList.add('open'); menuBtn.style.display = 'none'; });
closeBtn.addEventListener('click', () => { sidebar.classList.remove('open'); menuBtn.style.display = 'flex'; });
...
function getFeatureStyle(feature) {
    ...
    const bgStyle = { color: '#64748b', weight: 1.5, opacity: 0.7, fillOpacity: 0 };
...
(function initializeMap() {
    // UNGOK_DATA는 ungok_data.js에서 로드됨
    if (typeof UNGOK_DATA === 'undefined') {
        statusMsg.innerHTML = `<span style="color:#ef4444;">지도 데이터 로드 실패 (ungok_data.js 확인)</span>`;
        console.error('UNGOK_DATA not found');
        return;
    }

    const data = UNGOK_DATA;
    ...
    console.log('✅ 지도 초기화 완료 - 가족 땅:', Object.keys(myLandsDB).length, '필지');
})();
...
// 📌 PNU 목록 복사 (상세조회용) - 현재 표시된 땅의 PNU 리스트
const copyPnuListBtn = document.getElementById('copy-pnu-list-btn');
copyPnuListBtn.addEventListener('click', () => {
    let activeFilters = [];
    if (mainSwitch.checked) {
        ownerCheckboxes.forEach(cb => { if (cb.checked) activeFilters.push(cb.value); });
    }
    ...
    if (pnuList.length === 0) {
        alert("표시된 땅이 없습니다. 필터를 확인해주세요.");
        return;
    }

    const text = pnuList.join('\n');
    navigator.clipboard.writeText(text).then(() => {
        alert(`✅ ${pnuList.length}개 PNU 복사 완료!\n\n상세 데이터 조회 페이지에서 붙여넣기(Ctrl+V) 후 조회하세요.`);
    }).catch(() => {
        // 클립보드 실패시 텍스트 영역으로 대체
